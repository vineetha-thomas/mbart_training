# Script form Cody
# Lines modified by VTHOMA as denoted by comments
...
def get_args():
    ...
    # ADDED BY VTHOMA
    arg_parse.add_argument('--src',required=True,help='Source language id')
    arg_parse.add_argument('--dst',required=True,help='Destination language id')
...
def read_data(data_dir):
    ...
    #lang_id = 0 #int representing each language # commented VTHOMA
    lang_list = [args.src, args.dst] # Added VTHOMA
    lang_idx = 0 # Added VTHOMA
...
def write_sampled_set(sentences,lang_counts,alpha,output_dir,test_split_ratio):
    ...
    for lang in sentences:
        train = open('{}/{}'.format(output_dir,'train.{}'.format(lang)),'w') #Updated VTHOMA
        test = open('{}/{}'.format(output_dir,'test.{}'.format(lang)),'w') # Updated VTHOMA
